...
//		toggle every 1 second
		...
		if (btn_value)
		{
			if (!prev_value)
			{
				...
				if (next_LED_handle == NULL)
				{
					vTaskResume(red_led_handle);
					vTaskResume(yellow_led_handle);
					vTaskResume(onboard_led_handle);
					next_LED_handle = red_led_handle;
				} else
				{
					xTaskNotify(next_LED_handle, 0, eNoAction);
				}
			}
		}
...
//		toggle every 1 second
		...
		if (status == pdTRUE)
		{
			next_LED_handle = yellow_led_handle;
