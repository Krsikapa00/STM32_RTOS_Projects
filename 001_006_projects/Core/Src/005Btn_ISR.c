...
TaskHandle_t volatile next_LED_handle;
/* USER CODE END 0 */
...
//  status = xTaskCreate(btn_press_handler, "btn_press", 200, NULL, 4, &btn_press_handle);
//  configASSERT(status == pdPASS);


  status = xTaskCreate(red_led_handler, "red_led", 200, NULL, 3, &red_led_handle);
  configASSERT(status == pdPASS);

  status = xTaskCreate(yellow_led_handler, "yellow_led", 200, NULL, 2, &yellow_led_handle);
  configASSERT(status == pdPASS);

  status = xTaskCreate(onboard_led_handler, "onboard_led", 200, NULL, 1, &onboard_led_handle);
  configASSERT(status == pdPASS);

  next_LED_handle = red_led_handle;
...
void button_interrupt_handler(void)
{
	...
	BaseType_t returnedPriority;
	returnedPriority = 0;
	traceISR_ENTER();
	xTaskNotifyFromISR(next_LED_handle,0, eNoAction, &returnedPriority);

	portYIELD_FROM_ISR(returnedPriority);
...
//		toggle every 1 second
		...
		if (status == pdTRUE)
		{
			...
			next_LED_handle = yellow_led_handle; //Update next task to suspend
			HAL_GPIO_WritePin(EXT_LED_PORT, RED_LED_PIN, GPIO_PIN_SET); //Turn on light before suspending
			vTaskSuspend(red_led_handle); //suspend task
...
//		Toggle every 800 ms
		...
		if (status == pdTRUE)
		{
			...
			next_LED_handle = onboard_led_handle;
			HAL_GPIO_WritePin(EXT_LED_PORT, YELLOW_LED_PIN, GPIO_PIN_SET); //Turn on light before suspending
			vTaskSuspend(yellow_led_handle);
			portEXIT_CRITICAL();
